...
El proyecto se desarrolló utilizando el lenguaje **Python** y se emplearon bibliotecas como pandas y numpy para la manipulación de datos numéricos. Para la creación de gráficos, se utilizó la librería matplotlib.pyplot. Para presentar los graficos se uso **`Power BI`**.
...
Se llevó a cabo un análisis exhaustivo del año 2022, concluyendo que al examinar las tablas y gráficos, se evidencia un acceso más destacado a Internet en las provincias de Capital Federal (CABA) y Tierra del Fuego. En contraste, se observa que Formosa y Santiago del Estero muestran un acceso relativamente menor. Se realizo un dashboard que lo puedes encontrar en **`Servicio de Internet de Power BI`**.
...
Además, en el archivo de **`Power BI`** , se incluyen gráficos que representan los KPI realizados. El primero indica un incremento del 2%, el segundo muestra el KPI propuesto con un aumento del 50%, el tercer gráfico refleja un crecimiento del 100%, y, finalmente, el cuarto gráfico realiza una comparación entre los tres KPI, destacando las diferencias. entre un aumento del 2%, un incremento del 50% y un aumento del 100%.

En la última hoja se encuentra el **`Análisis Exploratorio de Datos (EDA)`** para el año 2022, desglosado por los cuatro trimestres.
